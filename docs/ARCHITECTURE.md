...
Every profile's `dotfiles/.config/nvim/` vendors the real, public
[LazyVim/starter](https://github.com/LazyVim/starter) template — a
Neovim + LazyVim setup ships built in, applied the same way as every
other dotfile (symlinked, backed up on first touch), no external repo
